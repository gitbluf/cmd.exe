 ...
 */
export function resolveModel(opts: ModelResolverOptions): Model<Api> | null {
	const {
		modelRegistry,
		currentModel,
		actionType = "main",
	...
	if (config?.overrides?.[actionType]) {
		...
		if (!config.fallback) {
			throw new Error(
				`Model not found: ${modelId} (override for action "${actionType}")`,
			);
		}
	}
...
function findModel(
	...
	// Exact match
	let model = available.find((m: Model<Api>) => m.id === modelId);
	if (model) return model;

	// Provider/id match (e.g., "openai/gpt-4o-mini")
	model = available.find((m: Model<Api>) => m.id.endsWith(`/${modelId}`));
	if (model) return model;
 ...
 */
export function createModelConfig(data: unknown): ModelConfig {
